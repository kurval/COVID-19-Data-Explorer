...
def world_cases(num_cases):
    ...
    st.markdown(
        ...
        f"<td style='border-right: 2px solid {_border_color}; border-spacing: 10px; {_number_format}' > {float(num_cases['new_cases']):,.0f}</td> "
        f"<td style='{_number_format}'> {float(num_cases['new_deaths']):,.0f} </td>"
        "</tr>"
        "</table>"
        "<br>",
